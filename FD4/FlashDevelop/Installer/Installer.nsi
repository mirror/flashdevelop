...
Section "Install AIR SDK" InstallAirSDK
	...
	NSISdl::download /TIMEOUT=30000 http://airdownload.adobe.com/air/win/download/3.2/AdobeAIRSDK.zip "$TEMP\air_sdk_${AIR}.zip"
	Pop $R0
	StrCmp $R0 "success" +4
...
Section "Install Flash Player" InstallFlashPlayer
	...
	IfFileExists "$INSTDIR\Tools\flexlibs\runtimes\player\11.2\win\*.*" +2 0
	CreateDirectory "$INSTDIR\Tools\flexlibs\runtimes\player\11.2\win\"
	...
	CopyFiles "$EXEDIR\flashplayer_11_sa_debug_32bit.exe" "$INSTDIR\Tools\flexlibs\runtimes\player\11.2\win\FlashPlayerDebugger.exe"
	Goto +9

	; Download Flash debug player
	NSISdl::download /TIMEOUT=30000 http://fpdownload.macromedia.com/pub/flashplayer/updaters/11/flashplayer_11_sa_debug_32bit.exe "$INSTDIR\Tools\flexlibs\runtimes\player\11.2\win\FlashPlayerDebugger.exe"
	Pop $R0
	StrCmp $R0 "success" +4
