...
//		printer.SetTabSize(settings.Pref_Flex_TabSize);
			...
			printer.SetWrapStyle(settings.Pref_MXML_WrapIndentStyle);
			printer.SetMaxLineLength(settings.Pref_MXML_MaxLineLength);
			printer.SetWrapMode(settings.Pref_MXML_AttrWrapMode);
		...
		public static void ConfigureASPrinter(ASPrettyPrinter printer, Settings settings, int tabSize)
		{
			...
			printer.SetBraceStyleSetting(settings.Pref_AS_BraceStyle);
...
//		printer.SetTrimTrailingWS(settings.Pref_AS_TrimTrailingWhitespace);
			...
			int wrapIndentStyle=settings.Pref_AS_WrapIndentStyle;
			WrapOptions options=new WrapOptions(settings.Pref_AS_WrapArrayDeclMode);
			options.BeforeSeparator = breakBeforeComma;
			options.IndentStyle = wrapIndentStyle;
			printer.SetArrayInitWrapOptions(options);

			options=new WrapOptions(settings.Pref_AS_WrapMethodCallMode);
			options.BeforeSeparator = breakBeforeComma;
			options.IndentStyle = wrapIndentStyle;
			printer.SetMethodCallWrapOptions(options);

			options=new WrapOptions(settings.Pref_AS_WrapMethodDeclMode);
			options.BeforeSeparator = breakBeforeComma;
			options.IndentStyle = wrapIndentStyle;
			printer.SetMethodDeclWrapOptions(options);

			options=new WrapOptions(settings.Pref_AS_WrapExpressionMode);
			options.BeforeSeparator = breakBeforeComma;
			options.IndentStyle = wrapIndentStyle;
			printer.SetExpressionWrapOptions(options);

			options=new WrapOptions(settings.Pref_AS_WrapXMLMode);
			options.BeforeSeparator = breakBeforeComma;
			options.IndentStyle = wrapIndentStyle;
