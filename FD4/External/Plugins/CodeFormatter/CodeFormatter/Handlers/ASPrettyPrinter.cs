...
//		mIsCompleteFile=isCompleteFile;
		...
		public String Print(int startIndent)
		{
			...
			mSourceData=CodeFormatter.Actionscriptinfocollector.Utilities.ConvertCarriageReturnsToLineFeeds(mSourceData);
...
//				mWorkingSource="class DummyClassWrapper {"+mSourceData+ "}";
				...
				while (true)
				{
					...
					{
						if (t.Type==AS3_exParser.PACKAGE)
							isPackageFound=true;
						break;
					}
				}
				...
				if (ActionScriptFormatter.ValidateNonWhitespaceIdentical(resultText, mWorkingSource))
				{
		...
		private static int GetEndEOLs(String source)
		{
			...
			for (int i=source.Length-1;i>=0;i--)
			{
				if ((source.ToCharArray())[i]=='\n')
					count++;
				else if (Char.IsWhiteSpace((source.ToCharArray())[i]))
					continue;
...
//		int newIndent=mCurrentIndent;
//		String text=buffer.ToString().trim(); //TODO: something more efficient
//		if (text.Length>0)
//		{
//			newIndent=Math.min(mCurrentIndent, lastIndent);
//			if (lastIndent<mCurrentIndent)
//			{
//				newIndent=lastIndent+mBlockIndent; //(mUnindent ? 0 : mBlockIndent);
//			}
//		}
//
//		return newIndent;
//	}
		...
		public void Emit(CommonToken tok)
		{
			...
			bool onBlankLine=ActionScriptFormatter.IsOnlyWhitespaceOnLastLine(mOutputBuffer); //I think I can only be on a blank line at the start of the document
			int blankLines=0;
				...
				switch (t.Channel)
				{
					...
					case AS3_exParser.CHANNEL_WHITESPACE:
						...
						else if (GetFormatMode()==FORMAT_INDENT)
						{
							...
							if ((commentTextOnLastLine || !IsFirstTokenBeforeFormatIndent()) && !ActionScriptFormatter.IsOnlyWhitespaceOnLastLine(mOutputBuffer))
								mOutputBuffer.Append(t.Text);
						}
						else
						{
						...
						if (IsGroupingChar(c))
						{
							...
							if (i>=0 && spaceCount>0) //only care if there are some spaces
							{
								...
								if (nestingType!=Nesting_Unrelated)
								{
									...
									{
										if (prevChar=='(' || c==')')
											amtToKeep=GetAdvancedSpacesInsideParens();
										else if (prevChar=='{' || c=='}')
									...
									{
										//ErrorManager.ShowInfo(mOutputBuffer.Length + "_" + (mOutputBuffer.Length-(spaceCount-amtToKeep)));
		...
		public bool IsHardIndent()
		{
			if (mIndentStack.Count>=1)
			{
				...
				if (indent.mType==BRACE_INDENT || indent.mType==INITIAL_INDENT)
					return true;
//	       int value=mIndentStack.get(mIndentStack.size()-2);
//	       if (value==(-1))
//	          return true;
			}
		...
		{
			if (mLastToken!=null && mLastToken.Text.Equals("{"))
				return true;
			return false;
		}
