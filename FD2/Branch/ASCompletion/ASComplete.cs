...
namespace ASCompletion
{
	...
	public class ASComplete
	{
		...
		static private readonly RegexOptions ro_csr =
			ASFileParser.ro_cs | RegexOptions.RightToLeft;
		...
		static private readonly Regex re_balancedBraces =
			ASFileParser.re_balancedBraces;
		static private readonly Regex re_balancedParenthesis =
			new Regex("\\([^()]*(((?<Open>\\()[^()]*)+((?<Close-Open>\\))[^()]*)+)*(?(Open)(?!))\\)", ASFileParser.ro_cs);
		//static private readonly Regex re_balancedBrakets =
		//	new Regex("\\[[^\\[\\]]*(((?<Open>\\[)[^\\[\\]]*)+((?<Close-Open>\\])[^\\[\\]]*)+)*(?(Open)(?!))\\]", ASFileParser.ro_cs);

		// expressions
		static private readonly Regex re_sub =
			new Regex("^#(?<index>[0-9]+)~$", ASFileParser.ro_cs);
			//new Regex("^#(?<index>[0-9]+)~(?<token>.*)$", ASFileParser.ro_cs);
		static private readonly Regex re_level =
			new Regex("^_level[0-9]+$", ASFileParser.ro_cs);
		#endregion
		...
		{
			if (autoHide && !ASContext.Context.HelpersEnabled)
				return false;
			try
			{
				...
				if (!ASContext.Context.IsFileValid()) return false;
				...
				switch (Value)
				{
					...
					case ':':
						ASContext.Context.UnsetOutOfDate();
		...
		static public bool OnShortcut(Keys keys, ScintillaNet.ScintillaControl Sci)
		{
			...
			else if (keys == (Keys.Control | Keys.Enter))
			{
				...
				{
					// quick build
					if (!ASContext.Context.BuildCMD(true)) ASContext.Context.HotBuild();
				}
				return true;
			}
			// help
		...
		static public bool DeclarationLookup(ScintillaNet.ScintillaControl Sci)
		{
			...
			if (!result.IsNull())
			{
				...
				{
					return ASContext.Context.BrowseTo(result.Class.ClassName);
				}

				// open/activate class file
				ClassModel oClass = (result.inClass != null) ? result.inClass : result.Class;
				if (oClass.IsVoid())
					return false;

				// for Back command
				ASContext.Panel.SetLastLookupPosition(ASContext.Context.CurrentFile.FileName, Sci.CurrentPos);

				// open the file
				ASContext.MainForm.OpenSelectedFile(oClass.InFile.FileName);
				...
				if (result.Member != null)
				{
					if ((result.Member.Flags & FlagType.Function) > 0)
						m = Regex.Match(Sci.Text, "function[\\s]+(?<mname>"+Regex.Escape(result.Member.Name)+")[\\s]*\\(", ASFileParser.ro_cs);
					else if ((result.Member.Flags & FlagType.Variable) > 0)
						m = Regex.Match(Sci.Text, "var[\\s]+(?<mname>"+Regex.Escape(result.Member.Name)+")[^\\w]", ASFileParser.ro_cs);
					else if ((result.Member.Flags & (FlagType.Getter | FlagType.Setter)) > 0)
						m = Regex.Match(Sci.Text, "function[\\s]+(?<mname>(g|s)et[\\s]+"+Regex.Escape(result.Member.Name)+")[\\s]*\\(", ASFileParser.ro_cs);
					// show
					if ((m != null) && m.Success)
					{
		...
		/// TODO  To be checked (toplevel related)
		/// </summary>
		/// <param name="Sci">Control</param>
		...
		static public Hashtable ResolveElement(ScintillaNet.ScintillaControl Sci, string eventAction)
		{
			...
			if (!result.IsNull())
			{
				...
				// CLASS DETAILS
				details.Add("@CLASSFILE", oClass.InFile.FileName);
				...
				if ((result.Class != null) && (result.Member != null))
				{
					...
					details.Add("@MEMBERDECL", ClassModel.MemberDeclaration(result.Member));
					//
					string kind = "";
					if ((result.Member.Flags & FlagType.Function) > 0)
						kind = "method";
					...
					//
					details.Add("@MEMBERCLASSFILE", result.Class.InFile.FileName);
					details.Add("@MEMBERCLASSDECL", ClassModel.MemberDeclaration(result.Class.ToMemberModel()));
				}
				else
				{
				...
				if (eventAction != null)
				{
					...
					{
						string cmd = ASContext.Context.HelpCommandPattern;
						if ((cmd == null) || (cmd.Length == 0)) return null;
						// top-level vars should be searched only if the command includes member information
						if ((result.inClass == ClassModel.VoidClass) && (cmd.IndexOf("@MEMBER") < 0)) return null;
						// complete command
		...
		static public bool HandleFunctionCompletion(ScintillaNet.ScintillaControl Sci)
		{
			...
			if (!result.IsNull())
			{
				...
				if (method == null)
				{
					...
					method = result.Class.Members.Search(constructor, FlagType.Constructor);
					if (method == null)
						return true;
				}
		...
		static private bool HandleDotCompletion(ScintillaNet.ScintillaControl Sci, bool autoHide)
		{
			    ...
			    ((expr.separator == ';') || (expr.separator == ' ') || (expr.separator == '}') || (expr.separator == '{')))
			{
				...
				{
					// TODO  Before & around class declaration completion
					/*if (Regex.IsMatch(text, "[\\s](class|interface)[\\s]"))
					{
						DebugConsole.Trace("Match class");
						Match m = Regex.Match(text, "[\\s](class|interface)[\\s]+"+ASContext.Context.CurrentClass.ClassName+"[\\s{]");
						if (m.Success)
						{
			...
			if ((expr.ContextFunction != null) || (expr.separator != ':'))
			{
				...
				bool limitMembers = ASContext.Context.HideIntrinsicMembers || (autoHide && !ASContext.Context.AlwaysShowIntrinsicMembers);
				...
				{
					tmpClass.Sort();
					mix.Merge(tmpClass.Members, mask);
					// TODO  improve: remove constructor methods in completion list
					foreach(MemberModel meth in tmpClass.Members)
					if ((meth.Flags & FlagType.Constructor) > 0) {
						mix.Remove(meth);
						break;
					}
					//mix.Merge(tmpClass.Properties, mask);
					//mix.Merge(tmpClass.Vars, mask);
					...
					if ((tmpClass != null) && ((tmpClass.Flags & FlagType.Intrinsic) == FlagType.Intrinsic)
					    && (tmpClass.InFile.FileName.StartsWith(ASContext.Context.TopLevelClassPath)))
					{
						if (limitMembers)
							tmpClass = null;
					}
			...
			{
				mix.Merge(cClass.ToMemberModel());
				mix.Merge(cClass.InFile.Imports);
				mix.Merge(ASContext.Context.TopLevel.Imports);
				mix.Merge(ASContext.Context.TopLevel.Members);
				mix.Merge(ASContext.Context.GetBasePackages());
			}
		...
		static private bool HandleNewCompletion(ScintillaNet.ScintillaControl Sci, string tail, bool autoHide)
		{
			...
			known.Merge(cClass.ToMemberModel());
			known.Merge(cClass.InFile.Imports);
			known.Merge(ASContext.Context.TopLevel.Imports);
			known.Merge(ASContext.Context.GetBasePackages());
			// show
			ArrayList list = new ArrayList();
		...
		static private bool HandleImportCompletion(ScintillaNet.ScintillaControl Sci, string tail, bool autoHide)
		{
			...
			// Show
			MemberList known = ASContext.Context.GetBasePackages();
			ArrayList list = new ArrayList();
			foreach(MemberModel member in known)
				list.Add(new MemberItem(member));
			CompletionList.Show(list, autoHide, tail);
		...
		static private bool HandleColonCompletion(ScintillaNet.ScintillaControl Sci, string tail, bool autoHide)
		{
			...
			if ((keyword != "var") && (keyword != "function") && (keyword != "get") && (keyword != "set"))
				return false;
		...
		static private ASResult EvalExpression(string expression, ASExpr context, ClassModel inClass, bool complete)
		{
			...
			if (token.StartsWith("#"))
			{
				...
				if (mSub.Success)
				{
					...
					if (head.Member != null)
						head.Class = ASContext.Context.ResolveClass(head.Member.Type, head.Class.InFile);
				}
				else
				{
			...
			// context
			ASResult step = head;
			ClassModel resultClass = head.Class;
			...
			if (step.Class.Flags == FlagType.Package)
			{
				...
				step.Class.Package = ASContext.Context.FindPackage(step.Class.ClassName, true);
			}
			return step;
		}
		...
		/// <param name="inClass">In class</param>
		/// <returns>Class/member struct</returns>
		static private ASResult EvalVariable(string token, ASExpr context, ClassModel inClass)
		{
			...
			foreach(MemberModel var in context.LocalVars)
			{
				if (var.Name == token)
				{
					result.inClass = null;
					result.Class = ASContext.Context.ResolveClass(var.Type, inClass.InFile);
					result.Member = var;
					return result;
				}
			}
			...
			if (context.ContextFunction != null)
			{
				...
				if (param.Success)
				{
					...
					if (param.Success && (param.Groups["type"].Value.Length > 0))
					{
						...
						result.Class = ASContext.Context.ResolveClass(param.Groups["type"].Value, inClass.InFile);
					}
					return result;
				}
			}
			...
			// TODO  top-level elements resolution
			/*if (!ASContext.Context.TopLevel.IsVoid())
			{
				FindMember(token, ASContext.Context.TopLevel, result, 0);
				if (!result.IsNull())
					return result;
				// special _levelN
				if (token.StartsWith("_") && re_level.IsMatch(token))
				{
					result.Class = ASContext.Context.ResolveClass("MovieClip", null);
					return result;
				}
			}*/
			...
			ClassModel aClass = ASContext.Context.ResolveClass(ctoken, inClass.InFile);
			if (!aClass.IsVoid())
			{
				result.Class = aClass;
				result.IsStatic = (p < 0);
			...
			// packages folders
			MemberList package = ASContext.Context.FindPackage(token, false);
			if (package != null)
			{
				result.Class = new ClassModel();
		...
		static private void FindMember(string token, ClassModel inClass, ASResult result, FlagType mask)
		{
			...
			if (p < 0)
			{
				...
				while ((tmpClass != null) && !tmpClass.IsVoid())
				{
					found = tmpClass.Members.Search(token, mask);
					if (found != null) break;
					...
					if (found != null)
					{
						...
						else
						{
							...
							result.Class = ASContext.Context.ResolveClass("Function", null);
			...
			{
				result.inClass = tmpClass;
				result.Class = ASContext.Context.ResolveClass(found.Type, tmpClass.InFile);
				result.Member = found;
				result.IsStatic = false; //((found.Flags & FlagType.Static) > 0);
			...
			else if (inClass.Package != null)
			{
				...
				result.Class = ASContext.Context.ResolveClass(inClass.ClassName.Replace(System.IO.Path.DirectorySeparatorChar,'.')+"."+token, null);
				if (!result.Class.IsVoid())
					return;

				// sub packages?
				MemberList list = ASContext.Context.FindPackage(inClass.ClassName, false);
		...
		static private ASExpr GetExpression(ScintillaNet.ScintillaControl Sci, int position)
		{
			...
			sb = null;
			expression.PositionExpression = position;
			...
			if (expr.Length > 0)
			{
				...
				expr = re_dot.Replace( re_whiteSpace.Replace(m.Value, " ") , ".").Trim();
				...
				//DebugConsole.Trace("Clean '"+expr+"' @"+expression.PositionExpression);
			}
		...
		{
			MemberList vars = new MemberList();
		...
		static public MemberList ParseMethodParameters(string parameters)
		{
		...
		/// </summary>
		static public bool FriendClasses(ClassModel aClass, ClassModel bClass)
		{
			ClassModel tmpClass = aClass;
			while (tmpClass != null)
			{
				if (tmpClass == bClass)
					return true;
		...
		static private MemberList GetContextClassesList(ClassModel cClass)
		{
			// current class
			MemberList list = new MemberList();
			list.Add(cClass.ToMemberModel());

			// inheritance
			ClassModel tmpClass = cClass.Extends;
			while(tmpClass != null && !tmpClass.IsVoid()
			    && !tmpClass.FileName.StartsWith(ASContext.TopLevelClassPath))
			{
				list.Add(tmpClass.ToMemberModel());
				tmpClass = tmpClass.Extends;
			}
			if (list.Count > 1)
				list.Sort();
			...
			if ((result.Member != null) && (result.inClass != null))
				{
					...
					// TODO  top-level tooltip text
					//if (result.inClass == ASContext.Context.TopLevel)
					//	text = text.Substring(0, text.IndexOf('\n'));
					return text;
				}
				else if ((result.Member != null) && ((result.Member.Flags & FlagType.Constructor) != FlagType.Constructor))
				{
					return ClassModel.MemberDeclaration(result.Member);
				}
				...
				{
					return ClassModel.ClassDeclaration(result.Class);
				}
				else return null;
		}
		...
		{
			if (type == 0 && ASContext.Context.AutoImportsEnabled && LastExpression != null)
			{
				// context
				if (LastExpression.Keyword != null && LastExpression.Keyword != "new")
					if (LastExpression.separator == ' ')
						return label;
		...
		{
			if (!ASContext.Context.AutoImportsEnabled)
				return false;
			...
			{
				// context
				DebugConsole.Trace("key "+LastExpression.Keyword+" '"+LastExpression.separator+"'");
				if (LastExpression.Keyword != null && LastExpression.Keyword != "new")
					if (LastExpression.separator == ' ')
						return false;

				ASResult context = EvalExpression(LastExpression.Value, LastExpression, ASContext.Context.CurrentClass, true);
