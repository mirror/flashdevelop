...
namespace WeifenLuo.WinFormsUI
{
	...
	public abstract class DockPaneStripBase : Control
	{
		...
		protected override void WndProc(ref Message m)
		{
			...
			else if (m.Msg == (int)Win32.Msgs.WM_MBUTTONDOWN)
			{
				
				int index = GetHitTest();
				if (index != -1)
				{
					if (Tabs[index].Content.ToString().StartsWith("FlashDevelop.TabbedDocument")) Tabs[index].Content.Close();
					else Tabs[index].Content.Hide();
				}
