...
namespace FlexDbg
{
    public class ScintillaHelper
    {
		...
		public const int indicatorDebugEnabledBreakpoint = 28;	// (Int32)ScintillaNet.Enums.IndicatorStyle.Max
		public const int indicatorDebugDisabledBreakpoint = 29;	// (Int32)ScintillaNet.Enums.IndicatorStyle.Max
		...
		static public void SciControl_MarginClick(ScintillaControl sender, int modifiers, int position, int margin)
		{
			...
			if (IsMarkerSet(sender, markerBPEnabled, line))
			{
				...
				sender.MarkerAdd(line, markerBPDisabled);
			}
			else if (IsMarkerSet(sender, markerBPDisabled, line))
			{
				sender.MarkerDelete(line, markerBPDisabled);
			}
			else
			{
        ...
        static public ITabbedDocument GetDocument(string filefullpath)
        {
            ...
            foreach (ITabbedDocument document in documents)
            {
                ...
                if (sci != null && filefullpath == sci.FileName)
					return document;
            }
