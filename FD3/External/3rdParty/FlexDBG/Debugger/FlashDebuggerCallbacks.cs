		...
		protected internal virtual String extractExenameFromCommandString(String cmd)
		{
			...
			else
			{
				...
				for (; ; )
				{
					...
					bool tmpBool;
					if (System.IO.File.Exists(new System.IO.FileInfo(filename).FullName))
						tmpBool = true;
					else
						tmpBool = System.IO.Directory.Exists(new System.IO.FileInfo(filename).FullName);
					if (tmpBool)
					{
