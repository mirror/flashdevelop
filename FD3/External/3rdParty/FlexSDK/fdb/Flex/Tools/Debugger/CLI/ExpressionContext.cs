...
namespace Flex.Tools.Debugger.CLI
{
	...
	{
		virtual internal Object Context
		{
			set
			{
				m_current = value;
		...
		virtual internal String CurrentPackageName
		{
			get
			{
				String s = null;
				try
				{
					Int32 o = (Int32) m_cache.get_Renamed(DebugCLI.LIST_MODULE);
		...
		internal ExpressionCache m_cache;
		internal Object m_current;
		internal bool m_createIfMissing; // set if we need to create a variable if it doesn't exist
		internal System.Collections.ArrayList m_namedPath;
		internal bool m_nameLocked;
		...
		*/
		...
		internal virtual long nameAsId(String name)
		{
			...
			{
				if (name[0] == '#')
					id = Int64.Parse(name.Substring(1));
			}
			catch (Exception)
			{
				id = Value.UNKNOWN_ID;
