...
package flash.net {
	...
	public class FileReference extends EventDispatcher {
		...
		/**
		 * Initiate uploading a file to a URL without any encoding. Whereas the upload() method encodes
		 *  the file in a form-data envelope, the uploadUnencoded() method passes the file contents as-is
		 *  in the HTTP request body. Use the uploadUnencoded() method if the data you wish to send is already encoded
		 *  in a format that the receiving server can understand.You typically use the uploadeUnencoded()
		 *  method with the HTTP/WebDAV PUT method.
		 *
		 * @param request           <URLRequest> The URLRequest object; the url property of the URLRequest object
		 *                            should contain the URL of the server script
		 *                            configured to handle upload through HTTP POST calls.
