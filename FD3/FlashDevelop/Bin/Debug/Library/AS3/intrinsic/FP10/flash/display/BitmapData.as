package flash.display
{
	...
	public class BitmapData extends Object implements IBitmapDrawable
	{
		...
		public function copyPixels (sourceBitmapData:BitmapData, sourceRect:Rectangle, destPoint:Point, alphaBitmapData:BitmapData, alphaPoint:Point, mergeAlpha:Boolean) : void;
		...
		public function histogram (hRect:Rectangle) : Vector.<Vector.<Number>>;

		/// Performs pixel-level hit detection between one bitmap image and a point, rectangle, or other bitmap image.
		public function hitTest (firstPoint:Point, firstAlphaThreshold:uint, secondObject:Object, secondBitmapDataPoint:Point, secondAlphaThreshold:uint) : Boolean;
		...
		public function noise (randomSeed:int, low:uint, high:uint, channelOptions:uint, grayScale:Boolean) : void;

		/// Remaps the color channel values in an image that has up to four arrays of color palette data, one for each channel.
		public function paletteMap (sourceBitmapData:BitmapData, sourceRect:Rectangle, destPoint:Point, redArray:Array, greenArray:Array, blueArray:Array, alphaArray:Array) : void;

		/// Generates a Perlin noise image.
		public function perlinNoise (baseX:Number, baseY:Number, numOctaves:uint, randomSeed:int, stitch:Boolean, fractalNoise:Boolean, channelOptions:uint, grayScale:Boolean, offsets:Array) : void;

		/// Performs a pixel dissolve either from a source image to a destination image or by using the same image.
		public function pixelDissolve (sourceBitmapData:BitmapData, sourceRect:Rectangle, destPoint:Point, randomSeed:int, numPixels:int, fillColor:uint) : int;
		...
		public function threshold (sourceBitmapData:BitmapData, sourceRect:Rectangle, destPoint:Point, operation:String, threshold:uint, color:uint, mask:uint, copySource:Boolean) : uint;
		...
		public function unlock (changeRect:Rectangle) : void;
	}
}
